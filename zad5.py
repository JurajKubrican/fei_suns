...
with graph.as_default():
    ...
    # LOGITS
    def model(data):
        layer = tf.nn.relu(tf.nn.conv2d(data, conv_weigths_1, [1, 2, 2, 1], padding='SAME') + conv_biases_1)
        # layer = tf.nn.relu(tf.nn.conv2d(layer, conv_weigths_2, [1, 1, 1, 1], padding='SAME') + conv_biases_2)
        layer = tf.nn.relu(tf.nn.conv2d(layer, conv_weigths_3, [1, 2, 2, 1], padding='SAME') + conv_biases_3)
